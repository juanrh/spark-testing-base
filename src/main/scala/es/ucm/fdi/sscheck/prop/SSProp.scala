...
import es.ucm.fdi.sscheck.gen.{DStream => TDStream}
...
// adding with StreamingSuiteBase gives an inheritanec error, investigate 
object SSPropTest extends Properties("example prop") {
  ...
  override def main(args: Array[String]): Unit = {
    println("running tests with custom params") 
    super.main(List("-minSuccessfulTests", "3").toArray)
   ...
   * */
  val runner = new StreamingSuiteBase {}  
  runner.beforeAll // this is needed to initialize the spark context, add some
  ...
  property("test avg alt") = 
    forAll ("inputDStream" |: inputDsGen, "expectedDStream" |: expectedOutDsGen) { 
      (inputDstream : TDStream[Int], expectedDstream : TDStream[Double]) =>
         ...
         * */
        runner.testOperation[Int, Int](inputDstream, identity[DStream[Int]] _,
                                       inputDstream, useSet = false)  
        true  
  }  
}
